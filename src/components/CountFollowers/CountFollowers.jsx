...
const CountFollowers = ({ id, followers, followingList, setFollowingList }) => {
  const initialCountFollowers = useMemo(() => {
    const storedCountFollower = JSON.parse(localStorage.getItem(`followerCount_${id}`));
  ...
  const formattedCountFollower = new Intl.NumberFormat('en-US', { style: 'decimal' }).format(countFollower);
  return (
    <>
      <p className={styles.follower}>{formattedCountFollower} followers</p>
      <FollowButton value={followingList} id={id} handleBtnClick={handleBtnClick}/>
    </>
  );
};
